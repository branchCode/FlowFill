...
class InpaintDataset(BaseDataset):
    ...
    def get_paths(self, opt):
        ...
        if opt.mask_type >= 3:
            mask_paths = [os.path.join(root_dir, '/data/zym/mask/testing_mask_dataset/{}.png'.format(str(i).zfill(5))) 
    ...
    def load_item(self, index):
        ...
        if self.opt.upsampler: # use samples instead of gt to train the upsampler 
            ...
            img_temp = Image.fromarray(cv2.blur(np.asarray(img), (10, 10)))
            img32 = img_temp.resize((32, 32), Image.BICUBIC)
            img32_tensor = F.to_tensor(img32)*2-1.
            # img32_tensor = color_quantize(img32_tensor)
